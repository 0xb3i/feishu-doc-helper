...
function createBlockRenderer(deps) {
  ...
  var UNSUPPORTED_NATIVE_BLOCK_TYPES = {
    whiteboard: true,
  };
  ...
  function isUnsupportedNativeBlockType(type) {
    return !!UNSUPPORTED_NATIVE_BLOCK_TYPES[String(type || '')];
  }
  ...
  function isContentfulRenderedBlock(snap, html, md) {
    if (!snap) return false;
    var type = String(snap.type || '');
    if (isUnsupportedNativeBlockType(type)) return false;
    if (/^heading[1-9]$/.test(type) || type === 'text' || type === 'ordered' || type === 'bullet' || type === 'todo') {
      return hasMeaningfulText(attribs.decodeBlockText(snap)) || hasMeaningfulText(md);
    }
    if (type === 'quote_container' || type === 'grid' || type === 'grid_column' || type === 'table_cell' || type === 'callout') {
      return hasMeaningfulText(attribs.decodeBlockText(snap)) || hasMeaningfulText(md) || hasMeaningfulText(String(html || '').replace(/<[^>]+>/g, ''));
    }
    if (type === 'table') return !!(snap.rows_id && snap.rows_id.length && snap.columns_id && snap.columns_id.length);
    if (type === 'image') return !!(snap.image && snap.image.token);
    if (type === 'divider' || type === 'code' || type === 'diagram' || type === 'whiteboard' || type === 'synced_reference') return true;
    return hasMeaningfulText(md) || hasMeaningfulText(String(html || '').replace(/<[^>]+>/g, ''));
  }
  ...
  function getImageAssetInfo(image, locationOrigin) {
    var src = image && image.token
      ? String(locationOrigin || '') + '/space/api/box/stream/download/preview/' + encodeURIComponent(String(image.token)) + '/?preview_type=16'
      : '';
  ...
  function blockToHtml(snap, block, childHtmlArr, context) {
    ...
    if (isUnsupportedNativeBlockType(type)) return '';
  ...
  function renderRootBlock(rootBlock, context) {
    ...
    function processTopBlock(block, depth) {
      ...
      if (block.record && block.record.snapshot) {
        ...
        if (isContentfulRenderedBlock(snap, html, md)) blockCount++;
        return;
      }
      getBlockChildren(block).forEach(function (child) { processTopBlock(child, depth + 1); });
    }
  ...
  function buildDocxRecordPayload(rootBlock, options) {
    ...
    function renderForContentCheck(block, depth) {
      if (!block || depth > maxDepth || !block.record || !block.record.snapshot) return { html: '', md: '' };
      var childHtml = [];
      var childMd = [];
      getBlockChildren(block).forEach(function (child) {
        var childRendered = renderForContentCheck(child, depth + 1);
        if (childRendered.html) childHtml.push(childRendered.html);
        if (childRendered.md) childMd.push(childRendered.md);
      });
      return {
        html: blockToHtml(block.record.snapshot, block, childHtml, { locationOrigin: '' }),
        md: blockToMarkdown(block.record.snapshot, block, childMd, { locationOrigin: '' }),
      };
    }
    ...
    function walk(block, depth, parentBlock) {
      ...
      if (block.record && block.record.id && block.record.snapshot) {
        var snap = block.record.snapshot;
        if (isUnsupportedNativeBlockType(snap.type)) return;
        ...
        if (cleanSnap.type === 'page') {
          ...
          if (isDirectChildOfPage) {
            var rendered = renderForContentCheck(block, depth);
            if (isContentfulRenderedBlock(cleanSnap, rendered.html, rendered.md)) {
